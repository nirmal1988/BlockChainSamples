...
func (t *SimpleChaincode) Query(stub  shim.ChaincodeStubInterface, function string, args []string) ([]byte, error) {
	...
	if function == "getVehicleByChassisNumber" { return t.getVehicleByChassisNumber(stub, args[0]) }
	if function == "getVehicleByVIN" { return t.getVehicleByVIN(stub, args[0]) }
...
func (t *SimpleChaincode) updateVehicle(stub  shim.ChaincodeStubInterface, args []string) ([]byte, error) {
	...
	if args[10] != "" {	
		...
		startDate = tt.Add(1*time.Minute).Local().String()
	...
	if bch.WarrantyStartDate != startDate {
		bch.WarrantyStartDate =  startDate
		updateStr += ",Warranty Start Date to "+ startDate
	}
